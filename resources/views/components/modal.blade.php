...
<div id="{{$id}}" uk-modal>
  <div class="uk-modal-dialog uk-modal-body uk-border-rounded modal">
    ...
    <div class="uk-flex uk-flex-right uk-margin-remove-bottom btn">
      <a class="uk-button uk-flex-last@s btn-modal-confirmar" onclick="event.preventDefault(); document.querySelector('form.form-logout').submit()">{{__('Confirmar')}}</a>
      <form action="{{route('logout')}}" class="form-logout" method="post" style="display:none;">
          @csrf
      </form>
